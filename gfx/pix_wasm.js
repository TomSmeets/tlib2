...
// gfx_wasm.js - Gfx implentation for WASM
tlib.exports.wasm_gfx_init = () => {
    tlib.canvas = document.getElementById('canvas')

    // Input listeners
    // TODO: why documenta and not canvas?
    document.addEventListener("keydown",   (ev) => { ctx.imports.wasm_key_down(ev.keyCode, true) })
    document.addEventListener("keyup",     (ev) => { ctx.imports.wasm_key_down(ev.keyCode, false) })
    document.addEventListener("mousemove", (ev) => { ctx.imports.wasm_mouse_move(ev.x, ev.y, ev.movementX, ev.movementY) })
    document.addEventListener("mousedown", (ev) => { ctx.imports.wasm_mouse_down(ev.button, true) })
    document.addEventListener("mouseup",   (ev) => { ctx.imports.wasm_mouse_down(ev.button, false) })
...
ctx.exports.wasm_gfx_begin_audio = () => {
    ctx.audio.resume();
}
